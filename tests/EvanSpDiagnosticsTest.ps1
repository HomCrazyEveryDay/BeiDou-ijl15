...
$output=Join-Path $env:TEMP ('evan-sp-diagnostics-'+[guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory $output | Out-Null
& cl.exe /nologo /EHsc /std:c++17 "$PSScriptRoot/EvanSpDiagnosticsTest.cpp" "/Fo$output\" "/Fe$output/test.exe"
