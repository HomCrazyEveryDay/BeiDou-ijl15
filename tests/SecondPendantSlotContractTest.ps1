...
Assert-SourceContains 'g_wvsContextSetExtraPendantSlot\(pThis,\s*packet\);(?s).*TryReadServerExpansionExpiration\(packet,\s*&serverExpiration\)(?s).*SyncSecondPendantExpiration\(pThis,\s*hasServerExpiration,\s*serverExpiration\);' 'toggle-first server expiration synchronization'
Assert-SourceContains 'packet->dataLength\s*-\s*packet->offset\s*<\s*sizeof\(\*expiration\)' 'trailing expiration bounds check'
Assert-SourceContains 'packet->offset\s*\+=\s*sizeof\(\*expiration\)' 'trailing expiration consumption'
