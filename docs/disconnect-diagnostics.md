...
当前设置快照版本的生命周期诊断由启动器设置控制，默认关闭，取代上文历史排查阶段的强制开启说明。启用后，`handshake_recv` 只记录已定位的原生握手调用点的请求长度、返回长度、错误码和 flags，不记录包体。原生握手重试循环有界。

`login_send_gate` 仅在登录与选角的指定操作码记录原生 socket 和 connecting 字段，辅助区分发送前被原生条件跳过的情形；不记录账号、密码或封包内容。该记录不能单独证明根因，游戏实机仍须由用户验证。
